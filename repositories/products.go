...
func ProductsRead(db *sql.DB, produtos []models.Product) ([]models.Product, error) {
	rows, erro := db.Query("SELECT id, name, description, price_cents, created_at FROM products WHERE is_active = true;")
	if erro != nil {
		return nil, erro
	}
	defer rows.Close()
...
func ProductDelete(db *sql.DB, id uint64) error {
	...
	result, erro := db.Exec("UPDATE products SET is_active = false WHERE id = $1;", id)
	if erro != nil {
		return erro
	}
